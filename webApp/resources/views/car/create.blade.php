...
@section('content')
    <div>
        <h2 class="text-black">{{ __('Add a vehicle') }}</h2>
        <div>
            <form method="post" action="{{ route("car.store") }}">
                @csrf
                <div>
                    <x-input-label for="plate" :value="__('plate')" />
                    <x-text-input id="plate" name="plate" type="text" class="mt-1 block w-full" required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="seats" name="seats" type="number" min="1" step="1" max="30" class="mt-1 block w-full" required />
                </div>
